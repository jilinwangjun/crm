...
public class SessionUserListener implements HttpSessionListener {
    ...
    private static java.util.Map<String, HttpSession> sessionMap = new java.util.concurrent.ConcurrentHashMap<String, HttpSession>(100);
    ...
    public static Map<Integer, String> getUserSessionMap() {
        ...
        while (iter.hasNext()) {
            ...
            int partyId = (Integer) session.getAttribute("partyId");
            if (Assert.isNotNull(partyId)) {
                map.put(partyId, sessionId);
            }
        }
        return map;
    }
     ...
     * @return
     */
    public synchronized static boolean checkIfHasLogin(int partyId) {
        Iterator<String> iter = getSessionMapKeySetIt();
        while (iter.hasNext()) {
            String sessionId = iter.next();
            HttpSession session = getSessionMap().get(sessionId);
            int sessionPartyId = (Integer) session.getAttribute("partyId");
            if (Assert.isNotNull(sessionPartyId)) {
                if (sessionPartyId == partyId) {
                    return true;
                }
            }
        }
